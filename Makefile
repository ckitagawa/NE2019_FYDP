###################################################################################################
# Modified from Midnight Sun's build system
#
# Arguments:
#   PL: [PLATFORM=] - Specifies the target platform (based on device family, defaults to stm32f4xx)
...
#   PB: [PROBE=] - Specifies which debug probe to use on STM32F4xx. Defaults to cmsis-dap [cmsis-dap | stlink-v2].
#
# Usage:
...
#
# Platform specific:
#   make gdb [PL=stm32f4xx] [PL] [PR] [PB]
#   make program [PL=stm32f4xx] [PR] [PB] - Programs and runs the project through OpenOCD
...
IGNORE_CLEANUP_LIBS := CMSIS FreeRTOS STM32F4xx_StdPeriph_Driver unity
