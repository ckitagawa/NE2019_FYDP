...
void stm32f4xx_interrupt_init(void) {
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);

  for (int i = 0; i < NUM_STM32F0XX_INTERRUPT_CHANNELS; i++) {
    s_stm32f4xx_interrupt_priorities[i] = NUM_INTERRUPT_PRIORITIES;
  }
}

StatusCode stm32f4xx_interrupt_nvic_enable(uint8_t irq_channel, InterruptPriority priority) {
  if (priority >= NUM_INTERRUPT_PRIORITIES || irq_channel >= NUM_STM32F0XX_INTERRUPT_CHANNELS) {
    return status_code(STATUS_CODE_INVALID_ARGS);
  } else if (s_stm32f4xx_interrupt_priorities[irq_channel] == priority) {
    return STATUS_CODE_OK;
  } else if (s_stm32f4xx_interrupt_priorities[irq_channel] != NUM_INTERRUPT_PRIORITIES) {
    return status_msg(STATUS_CODE_RESOURCE_EXHAUSTED, "Priority already set.");
  }

  s_stm32f4xx_interrupt_priorities[irq_channel] = priority;
...
StatusCode stm32f4xx_interrupt_exti_enable(uint8_t line, const InterruptSettings *settings,
                                           InterruptEdge edge) {
...
StatusCode stm32f4xx_interrupt_exti_trigger(uint8_t line) {
