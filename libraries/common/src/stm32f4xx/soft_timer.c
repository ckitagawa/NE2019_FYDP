...
StatusCode soft_timer_start(uint32_t duration_us, SoftTimerCallback callback, void *context,
  ...
  const uint32_t count = TIM_GetCounter(TIM5);
  ...
  if (head) {
    TIM_SetCompare1(TIM5, s_timers.head->expiry_us);
...
uint32_t soft_timer_remaining_time(SoftTimerId timer_id) {
  ...
  if (s_storage[timer_id].expiry_rollover_count > s_timers.rollover_count) {
    return UINT32_MAX - TIM_GetCounter(TIM5) + s_storage[timer_id].expiry_us;
  } else {
    return s_storage[timer_id].expiry_us - TIM_GetCounter(TIM5);
  }
}
...
static void prv_init_periph(void) {
  ...
  stm32f4xx_interrupt_nvic_enable(TIM5_IRQn, INTERRUPT_PRIORITY_NORMAL);
...
static void prv_update_timer(void) {
  ...
  while (active_timer != NULL && (active_timer->expiry_rollover_count < s_timers.rollover_count ||
                                  (active_timer->expiry_rollover_count == s_timers.rollover_count &&
                                   active_timer->expiry_us <= TIM_GetCounter(TIM5) + 10))) {
  ...
  if (s_timers.head != NULL) {
    ...
    uint32_t curr_time = TIM_GetCounter(TIM5);
    ...
    if (min_interval_rollover == next_expiry_rollover) {
      ...
      TIM_SetCompare1(TIM5, MAX(s_timers.head->expiry_us, min_interval_time));
    } else {
      ...
      TIM_SetCompare1(TIM5, (min_interval_rollover) ? min_interval_time : s_timers.head->expiry_us);
    }
    TIM_CCxCmd(TIM5, TIM_Channel_1, TIM_CCx_Enable);
  }
}
