...
static uint32_t s_gpio_rcc_ahb_timer_map[] = { RCC_AHB1Periph_GPIOA, RCC_AHB1Periph_GPIOB,
...
StatusCode gpio_init_pin(const GpioAddress *address, const GpioSettings *settings) {
  ...
  RCC_AHB1PeriphClockCmd(s_gpio_rcc_ahb_timer_map[address->port], ENABLE);
  ...
  init_struct.GPIO_Speed =
      GPIO_Speed_100MHz;  // Use fastest speed because the slew rate is quite slow.
