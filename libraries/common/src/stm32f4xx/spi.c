...
StatusCode spi_init(SpiPort spi, const SpiSettings *settings) {
  ...
  size_t index = 32 - (size_t)__builtin_clz(clocks.PCLK1_Frequency / settings->baudrate);
  if (index < 2) {
...
StatusCode spi_exchange(SpiPort spi, uint8_t *tx_data, size_t tx_len, uint8_t *rx_data,
  ...
  for (size_t i = 0; i < tx_len; i++) {
    ...
    SPI_SendData(s_port[spi].base, tx_data[i]);

    while (SPI_I2S_GetFlagStatus(s_port[spi].base, SPI_I2S_FLAG_RXNE) == RESET) {
    }
    SPI_ReceiveData(s_port[spi].base);
  }

  for (size_t i = 0; i < rx_len; i++) {
    while (SPI_I2S_GetFlagStatus(s_port[spi].base, SPI_I2S_FLAG_TXE) == RESET) {
    }
    SPI_SendData(s_port[spi].base, 0x00);

    while (SPI_I2S_GetFlagStatus(s_port[spi].base, SPI_I2S_FLAG_RXNE) == RESET) {
    }
    rx_data[i] = SPI_ReceiveData(s_port[spi].base);
  }
